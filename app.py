...
def preprocess_text(text):
    ...
    text = re.sub(r'[^\w\s]', '', text)  # Remove special characters but keep non-Latin scripts
    text = re.sub(r'\d+', '', text)  # Remove numbers

    # Remove stopwords and lemmatize for English words only
    ...
    try:
        for comment in comments:
            if len(comment.strip()) == 0:
                sentiments.append('neutral')  # Assign neutral to empty comments
            else:
                result = nlp(comment)
...
def main():
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            try:
                # Read the uploaded CSV file with utf-8 encoding to support Hindi script
